...
/**
 * Class that creates a bird object that inherits most of it's basic functionality from the Pets class.
...
public class Bird extends Pets {
	...
	public Bird(String givenName){
		...
		startWeight = weight;
		description = "A light cute pet that gains happiness very easily, but does not gain as much happiness from toys.";
		species = "Bird";
		favouriteFood = new Fruit();
		favouriteToy = new Slide();
	}
	...
	public String eat(Item f) {
		...
		weight += f.getRestoreValue() * 0.005;
	...
	public String playWithToy(Item t) {
		...
		if (!isDead){
			double birdMultiplier = 0.25; //specific animal multiplier based on the characteristics of the animal.
			if (t.equals(favouriteToy)){
				birdMultiplier = 0.5;
			}
			
			//loses energy from playing with the toy.
			double eneLoss = t.getRestoreValue()*0.35*birdMultiplier;
			if ((energy - eneLoss) < 0.005){
				...
				double hapGain = t.getRestoreValue() * birdMultiplier;
				happiness += hapGain;
				if (happiness > 100){
